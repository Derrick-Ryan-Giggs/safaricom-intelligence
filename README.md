...
- `assert_mpesa_revenue_consistency` — fails if `mpesa_metrics.mpesa_revenue_kes_bn` doesn't match `revenue_segments.mpesa_kes_bn` for the same period (both describe the same revenue line, sourced from different pages of the same booklet — they should tie out)
